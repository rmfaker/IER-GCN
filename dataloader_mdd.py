...
A copy of your original dataloader.py but swapping Reader to data.MDDParser.
This lets you reuse train_eval_evgcn.py with MINIMAL edits:
  - change "from dataloader import dataloader" -> "from dataloader_mdd import dataloader"
  - change "import data.ABIDEParser as Reader" -> "import data.MDDParser as Reader"
  - change LOSO site resolution: use Reader.get_subject_score(... 'SITE_ID') instead of resolve_sites_for_subjects()

Also, set the dataset root once:
  import data.MDDParser as Reader
  Reader.set_root(opt.mdd_root)

or export environment variable:
  export MDD_ROOT=/path/to/MDD_wwh_667
"""
...
class dataloader():
    ...
    def load_data(self, connectivity='correlation', atlas='cc200'):
        ...
        # Non-imaging covariates (keep ABIDE format: [site_index, sex, age])
    ...
    def get_PAE_inputs(self, nonimg):
        ...
        # static affinity score used to pre-prune edges (same as ABIDE)
